"""Contract files (`blastradius.contract.v1`): expected / allowed / forbidden
effects over a before/after captured pair. Parsing fails closed: unknown rule
names, unknown keys, conflicting count forms, duplicate effect ids, and
invariant rules outside `expected` are all hard errors.

Internally the parsed object is still called `Scenario` (the engine ported from
StateDiff reads it under that name); the public API in api.py exposes it as a
`Contract`, which is the vocabulary the CLI and docs use."""
...
def require_count_spec(where: str, spec: Any) -> None:
    ...
    if isinstance(spec, Bounds):
        ...
        if spec.max is None and spec.min is not None and spec.min <= 0:
            # `min: 0` is the same vacuous bound as `{}` spelled differently: no
            # count of rows is negative, so it admits everything. It is the more
            # dangerous spelling, because it looks deliberate and it is the one
            # that silently turns an `allowed` entry into a blanket waiver.
            raise ScenarioError(
                f"{where} wants at least {spec.min}, which every count already "
                "satisfies, so it bounds nothing; state a real lower bound or a max"
            )
...
def require_encodable_text(where: str, value: Any) -> None:
    """Refuse a string no captured state can carry."""
...
class Provenance(BaseModel):
    ...
    def _check(self) -> "Provenance":
        ...
        require_encodable_text("provenance.task", self.task)
        return self
...
class Scenario(BaseModel):
    model_config = ConfigDict(extra="forbid")
    contract: Literal["blastradius.contract.v1"]
    id: str
    title: str
    # An optional hint naming the adapter this contract was written against (for
    # example "sql" or "silobench"). It is not a pin: the same effects can grade a
    # capture from any adapter whose schema carries the tables and columns they
    # name, so the runtime adapter choice is what actually decides parsing.
    ...
    def _check(self) -> "Scenario":
        ...
        # the verdict JSON as `contract_id`/`title`. Pydantic refuses a lone
