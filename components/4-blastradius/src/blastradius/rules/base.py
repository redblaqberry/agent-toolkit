...
from ..adapter import ArtifactPair
...
class RuleContext:
    ...
    def schema(self) -> Schema:
        """The schema the adapter declared for this capture. Rules read tables,
        columns, primary keys, and event types off it, so the same rule works
        against any store an adapter supports."""
        return self.pair.after.schema

    def columns(self, table: str) -> frozenset[str]:
        return self.schema.columns[table]

    def primary_key(self, table: str) -> str:
        return self.schema.primary_keys[table]
...
def require_table(ctx: RuleContext, rule_id: str, table: str) -> None:
    ...
    if table in ctx.schema.non_diffed_tables:
        raise ScenarioError(
            ...
            "and any bookkeeping tables by the adapter's consistency invariants"
        )
    if not ctx.schema.has_table(table):
        raise ScenarioError(
            f"effect '{rule_id}': unknown table {table!r} "
            f"(known: {sorted(ctx.schema.columns)})"
        )
...
def require_event_type(ctx: RuleContext, rule_id: str, event_type: str) -> None:
    ...
    #
    # When the adapter declares no fixed vocabulary (event_types is None), any
    # type is accepted: a store with an open-ended audit log should not have to
    # enumerate every event kind up front.
    known = ctx.schema.event_types
    if known is not None and event_type not in known:
        raise ScenarioError(
            f"effect '{rule_id}': unknown event type {event_type!r} (known: {sorted(known)})"
        )
