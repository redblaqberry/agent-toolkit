...
`examples/refund_handling/` exists to keep the previous paragraph honest. It is a second contract from a different business: refund handling at an online electronics retailer, with its own transcript, its own actions (`read_order`, `check_return_window`, `check_fraud_signals`, `issue_refund`, `send_customer_message`), its own roles, its own escalation triggers, its own unit of work (a refund request, not an invoice), and its own out-of-band promise (an immutable decision log). It compiles to ten scenarios with no change to any code. `tests/test_second_domain.py` serializes that export and fails if a single word from the invoice example (`invoice`, `erp`, `vendor`, `ap_approver`, `posting`, and the rest) appears anywhere in it, so the compiler cannot quietly acquire a template again.
