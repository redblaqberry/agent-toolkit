...
def _evaluate(scenario: Scenario, pair: ArtifactPair) -> Verdict:
    ...
    environment_table = pair.after.schema.environment_table
    unexplained: list[dict] = []
    waived: list[dict] = []
    for atom in diff.atoms:
        if environment_table is not None and atom.table == environment_table and profile_change_allowed:
            # the waiver removes these from the sweep, so they are recorded and
            # reported rather than dropped: a change nobody checked must never
            # leave the run looking like a change nobody made
            waived.append(atom.describe())
            continue
    ...
    if unexplained:
        failed = True
    if not unexplained:
        detail = "every change is justified by an expected or allowed effect"
        if waived:
            # the sweep did not cover everything, so it does not get to say it did
            detail = (
                f"every change outside {environment_table} is justified by an "
                f"expected or allowed effect; {len(waived)} change(s) to "
                f"{environment_table} were waived by profile_change: allow and "
                f"were NOT checked"
            )
    else:
        detail = f"{len(unexplained)} change(s) no expected or allowed effect accounts for"
    checks.append(CheckOutcome(
        id="unexplained", rule="unexplained_sweep", list_name="invariant",
        status=sweep_status, detail=detail, evidence=unexplained,
    ))
    if waived:
        # a named check, so the waiver is visible in the gate JSON a CI consumer
        # reads, instead of being an absence they would have to notice
        checks.append(CheckOutcome(
            id="profile_change_waived", rule="profile_change_waiver",
            list_name="invariant", status="not_applicable",
            detail=(
                f"profile_change: allow removed {len(waived)} change(s) to "
                f"{environment_table} from the sweep; nothing here was verified"
            ),
            evidence=waived,
        ))

    return Verdict(
        contract_id=scenario.id,
        title=scenario.title,
        status="fail" if failed else "pass",
        checks=checks,
        unexplained=unexplained,
        artifacts=ArtifactFingerprints(
            schema_name=pair.after.schema.name,
            before_state_hash=pair.before.state_hash,
            after_state_hash=pair.after.state_hash,
