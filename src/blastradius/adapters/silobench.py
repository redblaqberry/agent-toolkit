...
A faithful port of StateDiff's SiloBench loader onto BlastRadius's pluggable
adapter boundary. StateDiff hardcoded one enterprise's tables, keys, columns,
and event types as module globals and returned its own snapshot object; here the
same knowledge is packed into a BlastRadius `Schema` the adapter declares, and
the loader hands the engine a `NormalizedSnapshot`. The engine, diff, and rules
never learn the state came from SiloBench.

The snapshot.v1 / event-log.v1 pydantic models live in THIS file: in BlastRadius
`models.py` holds only the generic engine models, so a store's native format
belongs to its adapter. The parsed domain event and the raw row type are the
generic ones, imported from `blastradius.models`.

Everything here fails closed: any structural surprise, hash mismatch, or
internal inconsistency raises the engine's ArtifactError, which the engine
reports as an `error` verdict (never a pass).
"""
...
# --------------------------------------------------------------------------
# snapshot.v1 / event-log.v1 format models (SiloBench's native artifacts).
# Copied from StateDiff's models.py; they reuse the generic EventRecord/Row.
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# SiloBench schema knowledge (from the SiloBench DDL).
# --------------------------------------------------------------------------
...
# against these at load time, and contract selectors are validated against
...
def _schema(version: int) -> Schema:
    """Build the BlastRadius Schema for a SiloBench export schema version.

    This is the whole inversion: the tables/keys/columns/event-types StateDiff
    read off module globals are packed into the Schema the engine reads off the
    captured state. `environment` is deliberately NOT in non_diffed_tables (a
    profile change must be justified or waived by profile_change=allow); only
    events and the bookkeeping counters sit outside the generic diff.
    """
    return Schema(
        primary_keys=dict(PRIMARY_KEYS[version]),
        columns=dict(COLUMNS[version]),
        event_types=EVENT_TYPES,
        non_diffed_tables=frozenset({EVENTS_TABLE, *BOOKKEEPING_TABLES}),
        events_table=EVENTS_TABLE,
        environment_table=ENVIRONMENT_TABLE,
        name=f"silobench.v{version}",
    )


@dataclass(frozen=True)
class _LoadedSnapshot:
    """A fully verified snapshot plus its parsed events, kept internally so
    `load_pair` can run the seed/schema/mutation_seq consistency checks and the
    event-log cross-validation against the raw SiloBench meta before the state
    is narrowed to the engine's NormalizedSnapshot."""
...
# --------------------------------------------------------------------------
# Strict JSON decoding (fail-closed at the single decode point).
# --------------------------------------------------------------------------
...
def _reject_duplicate_keys(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    the rewrite. The contract YAML loader already refuses duplicate keys for
    the same reason.
    """
    seen: dict[str, Any] = {}
...
def _require_json_domain(what: str, value: Any) -> None:
    ...
    an encoding traceback instead of the machine-readable refusal blastradius
...
# --------------------------------------------------------------------------
# Hashing and structural verification.
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Loading (snapshot -> NormalizedSnapshot) and event-log cross-validation.
# --------------------------------------------------------------------------


def _load_verified(path: str | Path) -> _LoadedSnapshot:
    path = Path(path)
    raw = _read_json(path)
    try:
    ...
    return _LoadedSnapshot(snapshot=snapshot, events=events, source=source)


def _to_normalized(loaded: _LoadedSnapshot) -> NormalizedSnapshot:
    """Narrow a verified SiloBench snapshot to the store-agnostic view the
    engine reads. raw_events is the stored event rows exactly as exported (each
    payload still the raw payload_json string): the append-only invariant
    compares THESE, so a reformatted or key-reordered payload_json in history is
    a rewrite even when the parsed event object would compare equal."""
    snapshot = loaded.snapshot
    return NormalizedSnapshot(
        schema=_schema(snapshot.meta.schema_version),
        tables=snapshot.tables,
        events=loaded.events,
        raw_events=snapshot.tables.get(EVENTS_TABLE, []),
...
def load_snapshot(path: str | Path) -> NormalizedSnapshot:
    """Load, fully verify, and normalize one snapshot.v1 artifact.

    Runs every fail-closed check (table set, row shapes and cell domain, the
    three hashes, event parsing and ordering, counters, environment) and only
    then hands back the NormalizedSnapshot the engine consumes.
    """
    return _to_normalized(_load_verified(path))
...
def cross_validate_event_log(loaded: _LoadedSnapshot, log: EventLog, source: str) -> None:
...
# --------------------------------------------------------------------------
# The adapter the engine plugs in.
# --------------------------------------------------------------------------


class SilobenchAdapter:
    """Loads an already-captured before/after SiloBench pair for the engine.

    Satisfies the StateAdapter protocol (`name` + `load_pair`). It does not
    implement `capture()`: a SiloBench snapshot is an offline export, not a live
    store the harness can snapshot in place.
    """

    name = ADAPTER_NAME

    def load_snapshot(self, path: str | Path) -> NormalizedSnapshot:
    ...
    def load_pair(
        ...
        before_loaded = _load_verified(before)
        after_loaded = _load_verified(after)
        before_meta = before_loaded.snapshot.meta
        after_meta = after_loaded.snapshot.meta
        if before_meta.seed != after_meta.seed:
            raise ArtifactError("before and after snapshots come from different seeds")
        if before_meta.schema_version != after_meta.schema_version:
            raise ArtifactError(
                "before and after snapshots have different schema versions; "
                "cross-schema diffing is out of scope for v0.1"
            )
        if after_meta.mutation_seq < before_meta.mutation_seq:
            raise ArtifactError("after snapshot has a lower mutation_seq than before")
        # The event logs are optional inputs. A False cross_checked flag means
        # the check never ran, which is NOT the same as passing it, so the flags
        # below record only which checks actually happened.
        if before_events is not None:
            cross_validate_event_log(before_loaded, load_event_log(before_events), str(before_events))
        if after_events is not None:
            cross_validate_event_log(after_loaded, load_event_log(after_events), str(after_events))
        return ArtifactPair(
            before=_to_normalized(before_loaded),
            after=_to_normalized(after_loaded),
            before_events_cross_checked=before_events is not None,
            after_events_cross_checked=after_events is not None,
        )


_DEFAULT_ADAPTER = SilobenchAdapter()
...
def load_pair(
    ...
    """Module-level convenience: delegate to a default SilobenchAdapter."""
    return _DEFAULT_ADAPTER.load_pair(before, after, before_events, after_events)
